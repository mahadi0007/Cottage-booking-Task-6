...
//		response.getWriter().append("Served at: ").append(request.getContextPath());
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("reqType").toString().equals("doQuery")) {
			...
			params.setMaxDayShifts(request.getParameter("maxDayShifts"));
			params.setisAvailableFrom((request.getParameter("isAvailableFrom")));

//			String pathToDB = this.getServletContext().getRealPath("/res/Cottages.ttl");
			String pathToDB = this.getServletContext().getRealPath("/res/cottage_individuals2.ttl");
